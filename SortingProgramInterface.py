...
def validate_array_length(text):
    try:
        res = int(text)
        if res <= 0:
            return False
...
def bubbleSort(my_list, up_down):
...
def selectionSort(my_list, up_down):
    for i in range(len(my_list)):
        min_index = i
        for j in range(i + 1, len(my_list)):
            if (up_down == 0 and my_list[j] < my_list[min_index]) or (up_down == 1 and my_list[j] > my_list[min_index]):
                min_index = j
        my_list[i], my_list[min_index] = my_list[min_index], my_list[i]
    return my_list


def insertionSort(my_list, up_down):
...
def cocktailSort(my_list, up_down):
