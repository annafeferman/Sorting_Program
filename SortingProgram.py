...
class DataInput:
    ...
    def getUpDown(self):
        while True:
            try:
                upDown = int(input('\nAscending list - input "1"\n' # выбор по возрастанию или убыванию
        ...
        for k in range(last_item):  # вложенным циклом перебираем все элементы массива
            for j in range(last_item - k):
                if (up_down == 1 and my_list[j] > my_list[j + 1]) or (up_down == 2 and my_list[j] < my_list[j + 1]):
                    comparisons += 1 #считаем сравнения
                    my_list[j], my_list[j + 1] = my_list[j + 1], my_list[j]
                    changes += 1 # считаем обмены

        print('Sorted list: ', my_list)
        print('Comparisons: ', comparisons, '  Changes: ', changes)
        print('This is bubble sort') # выводим название метода сортировки, чтобы пользователь наглядно видел
...
class SelectionSort:
    ...
    def sortedArray(self, my_list, up_down):
        ...
        print('This is selection sort') # выводим название метода сортировки
...
class InsertionSort:
    ...
    def sortedArray(self, my_list, up_down):
        ...
        print('Sorted list: ', my_list)
        print('Comparisons: ', comparisons, '  Changes: ', changes)  # сравнения и обмены
        print('This is insertion sort') # выводим название метода сортировки, чтобы пользователь наглядно видел
...
class CocktailSort:
    ...
    def sortedArray(self, my_list, up_down):
        ...
        print('Sorted list: ', my_list)
        print('Comparisons: ', comparisons, '  Changes: ', changes)  # сравнения и обмены
        print('This is cocktail sort') # выводим название метода сортировки
...
class ShellSort:
    ...
    def sortedArray(self, my_list, up_down):
        ...
        print('Sorted list: ', my_list)
        print('Comparisons: ', comparisons, '  Changes: ', changes)  # сравнения и обмены
        print('This is shell sort') # выводим название метода сортировки


class RunProgram:

    def SortMethod(self): # метод считывает ранее введенное значение переменной method и использует соответствующий метод сортировки
    ...
    def Run(self):
        while True:
            ...
            sortMethod = self.SortMethod(RunProgram)
            upDown = DataInput.getUpDown(DataInput)
            print('\nOld list: ', oldList)
            b = sortMethod()
            b.sortedArray(oldList, upDown)
            print('Program execution time is ', timeit.timeit(number=100000)) # рассчитываем время выполнения программы
            repeat = input('\nInput 1 if you want to restart the program. If you do not, input anything else: ')
            if repeat == '1':  # пользователь выбирает повторить ли программу
                continue
            else:
                break
